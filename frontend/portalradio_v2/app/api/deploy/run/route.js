...
function getCommands() {
    ...
    return {
        ...
        'stop-claude': {
            cmd: 'bash',
            args: ['-c', [
                'PID=$(cat /tmp/portal-radio.pid 2>/dev/null)',
                'if [ -n "$PID" ] && kill -0 $PID 2>/dev/null',
                'then kill $PID && rm -f /tmp/portal-radio.pid && echo "Claude Code stopped (PID $PID)"',
                'else pkill -f "claude" 2>/dev/null && echo "Claude Code processes killed" || echo "No Claude Code process found"; rm -f /tmp/portal-radio.pid',
                'fi',
            ].join('; ')],
            cwd: root,
        },
