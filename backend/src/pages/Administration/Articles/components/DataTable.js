...
const Index = () => {
    ...
    const defaultUrl = store.url + '/articles/node/' + parentId // set the index url to /api/articles/node/{parentId}
    const [items, setItems] = useState([]) // data placeholder

    // to get items data
    //console.log(defaultUrl)
    useEffect(() => {
        // Fetch data with modified axios
        axios({
            method: 'get',
            url: store.getValue('url') || defaultUrl // Use store.getValue('url') if available, otherwise defaultUrl
        })
        .then(response => {
    ...
    [
        store.getValue('url'), // listen for URL changes by pagination click
        store.getValue('refresh'), // listen when create / update / delete / search performed
        parentId // when navigating to a different parent
    ]); // useEffect()
